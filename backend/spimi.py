...
class SPIMI:
    ...
    def load_norms(self):
        with open('normas.json', 'r', encoding='utf-8') as file:
            data = json.load(file)
    ...
    def construirSpimi(self, limite_mb_bloque):
        ...
        with open('normas.json', 'w', encoding='utf-8') as f:  # guardar normas en un json
    ...
    def merge(self):
        ...
        for bloque_id in range(num_bloques):
            ...
            os.remove(os.path.join(self.pathTemp, f'bloque_{bloque_id}.json'))
    ...
    def similitudCoseno(self, query, k):
        ...
        for token in query_tokens:
            for bloque in bloques_relevantes:
                with open(bloque, 'r', encoding='utf-8') as f:
                    bloque_data = json.load(f)
                    if token in bloque_data:
                        if token not in posting_lists:
                            posting_lists[token] = []
                        posting_lists[token].extend(bloque_data[token])

        # Ordenar las listas de postings por doc_id para búsqueda binaria
        for token in posting_lists:
            posting_lists[token] = sorted(posting_lists[token], key=lambda x: x[0])
        ...
        # 4.2 TF-IDF para los documentos relevantes con búsqueda binaria
        tf_idf_docs = defaultdict(float)
        for token, postings in posting_lists.items():
            df = len(postings)
            for doc_id, tf in postings:
                idx = bisect_left([p[0] for p in postings], doc_id)
    ...
    def busqueda_topK(self, query, k=5):
        ...
        doc_ids = [pair[0] - 2 for pair in top_k_results]
...
# Uso
spimi = SPIMI(csv_path='./data/spotify_songs.csv')
query = 'hello'
